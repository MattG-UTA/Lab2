...
int main (int argc, char*argv[])
{
	...
	for (i=m-1;i>=n;i--) length[left[i]] = length[middle[i]] = length[right[i]] = length[i] +1;
	...
	for (i=0;i<n;i++)
	{
  		...
  		else if(middle[parent[j]] == j) outString[length[j]] = '1'; 
